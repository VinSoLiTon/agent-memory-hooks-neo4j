...
| **D** — Typed memory + admission gate | 🔵 in progress (#13, #16, #22, #29) | #13, #16, #22, #29 | PR-1 `:EXTRACTED_FROM`; PR-2 A-MAC grounding gate (acc #2); PR-3 D3 **retrieval eval** (`eval_retrieval.py`; `njhook eval-retrieval`; acc #3 retrieval half); PR-4 (D1) **typed `kind` vocabulary** — `hooks/memory_types.py` (15 semantic types ALL ACTIVE, `kind` replaces bucket labels which move to the PATH axis; legacy migration window via `normalize_kind`); first-class enum in `DREAM_JSON_SCHEMA`, validated at the boundary, stamped as `m.kind` node property; `njhook stats` by bucket+kind (acc #1 + #4); 3+3+1+9 tests | `njhook migrate-kinds` bulk re-tag (data migration); D3 **distillation** eval (acc #3 second half); model-cited attribution upgrade |
...
**Rollup:** **all 8 phases A–H now touched.** Complete & fully aligned: **A, B, C, E, F, G, H** (7 of 8). In progress: **D** only — `EXTRACTED_FROM` + grounding gate + anti-poisoning + retrieval eval are shipped; remaining = typed `kind` vocabulary (needs the deferred design) + the LLM **distillation** eval (the one piece that resists a deterministic gate). Both north-star halves — *trustworthy evolution-tracing* and *universal layer* — are working end-to-end.
...
| #29 | open | Phase D (PR-4 / D1) — typed `kind` vocabulary (`memory_types.py`, 15 active, replaces bucket labels, migration window, schema enum + `m.kind` property); acceptance #1 + #4 met |

## Metrics

- Tests: **19 → 158** over the program (live Neo4j + pure).
