...
| **D** — Typed memory + admission gate | 🔵 in progress (#13, #16, #22, #29, #30) | #13, #16, #22, #29, #30 | PR-1 `:EXTRACTED_FROM`; PR-2 A-MAC grounding gate (acc #2); PR-3 D3 **retrieval eval** (acc #3 retrieval half); PR-4 (D1) **typed `kind` vocabulary** (`memory_types.py`, 15 active, replaces bucket labels, schema enum + `m.kind` property; acc #1 + #4); PR-5 **`njhook migrate-kinds`** (audited, idempotent re-tag of legacy rows + property backfill; live graph already on semantic kinds → no-op); 3+3+1+9+6 tests | D3 **distillation** eval (acc #3 second half — LLM-in-the-loop); model-cited attribution upgrade |
...
| #30 | open | Phase D (PR-5) — `njhook migrate-kinds` (audited, idempotent re-tag of legacy rows + `m.kind` backfill) |

## Metrics

- Tests: **19 → 164** over the program (live Neo4j + pure).
