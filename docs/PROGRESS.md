...
| **B** — Durable capture (spool/inbox/DLQ) | ✅ merged & fully aligned | #11, #28 | PR-1: append-only fsync spool + `njhook ingest` worker + idempotent replay (Event.event_id = inbox) + DLQ + health backlog row; `HOOKS_CAPTURE_MODE=spool` (default `direct`); PR-2: **canonical schema** (`event_schema.py` — single-source version, OTel `gen_ai.*` map + `to_gen_ai`, **read-time upcaster chain**), schema **v2** makes `app_id` a first-class Event property (was dropped on ingest), **DLQ-rate** health (`dlq_rate_per_hour`; FAIL on rate, not static count); 6+12 tests | default-flip→spool deferred (needs *scheduled* ingest; not an acceptance bar — direct stays available per acc #6) |
...
| **D** — Typed memory + admission gate | ✅ merged & fully aligned | #13, #16, #22, #29, #30, #31 | PR-1 `:EXTRACTED_FROM`; PR-2 A-MAC grounding gate (acc #2); PR-3 D3 **retrieval eval** (acc #3 retrieval); PR-4 (D1) **typed `kind` vocabulary** (15 active, replaces bucket labels, schema enum + `m.kind` property; acc #1 + #4); PR-5 **`migrate-kinds`** (audited re-tag + backfill); PR-6 D3 **distillation eval** (`eval_distillation.py` deterministic scorer CI-gated + opt-in `njhook eval-distillation` provider matrix; acc #3 distillation) — **all 4 acceptance bars met**; 3+3+1+9+6+9 tests | model-cited attribution (follow-on, not a bar) |
...
**Rollup:** 🎉 **PROGRAM COMPLETE — all 8 phases A–H done & fully aligned (A, B, C, D, E, F, G, H).** Every numbered acceptance bar across the plan is met with tests + live verification and merged to `main`. Both north-star halves — *trustworthy evolution-tracing* (Phase F UI + Phase A history + Phase H2 audit) and *universal layer* (Phase G CLI/REST/MCP/renderers) — are working end-to-end. Remaining items are explicitly out-of-acceptance follow-ons (Phase B default-flip→spool pending a scheduled ingest; D model-cited attribution; full confidence-annealing; C4 reranker) — all documented in the deviations log, none blocking alignment.
...
- **Project-scoped recall + prompt budget** (#72, 2026-07-18): live Claude Code sessions received foreign-project memories on nearly every prompt (soft RRF boost only) at 4k chars/injection. Fix = `INJECT_PROJECT_SCOPE=hard` drop rule in `hybrid_merge` (path-scoped to `project/` — Neo4j data shows profile/tools carry no project tag while 31/34 general/ rows do and must keep flowing) + `render_prompt` budgeted by `INJECT_CHAR_BUDGET` with an oversized-top-hit clamp. Hard mode ships only via the deployed wrapper env (`.claude/hooks/inject_memory.cmd`/`.sh`: hard, 1800 chars, 3/2/2 bucket limits, boost 2.0); code default stays `soft` so dashboard/CLI are byte-identical. E2E-verified: ed-session leak (`project/njhook-roadmap-status.md`) reproduced in baseline, gone via wrapper; scoping follows cwd. Drive-by test hygiene: defused a time-bomb fixture (`test_recall_cohorts` hardcoded `ingested_at` aged past the 30-day cutoff) and de-corpus-coupled `test_update_gate` (fixture novelty decayed to 0.5 < 0.6 gate threshold as the live general/ corpus grew).
- **Memory-hygiene guards** (#73, 2026-07-18): root-caused the `project/njhook.md` corruption class — rendered-injection output (`## <path>` block headers + framing markers) written back into memory bodies by writers outside the dream gate, compounding per re-injection cycle (4 stacked self-headers, 3x bloat, regeneration drift). A health scan found the class was systemic: **31 more active memories** carried the same leading-header echo (verified genuine, zero false positives). Shipped: `hooks/content_guard.py` (deterministic artifact detection) wired into ALL sanctioned write doors (`quality.validate_memory` for dream, `service.propose_memory` for MCP/REST, `njhook edit`); `embeddings.memory_text` capped at `EMBED_TEXT_MAX_CHARS=4000` (an oversized 7.5k body 500'd the embedding server and aborted `embed-backfill` mid-run, silently stranding 252/380 embeddings — the pre-existing `EMBED_MAX_CHARS=6000` cap inside `embed()` was above the model's real limit); two new health rows (`memory hygiene` artifact scan + `embedding coverage`). Live cleanup performed (audited `:MemoryRevision` snapshots, reversible): njhook.md hand-rewritten (7430→1738 chars, drift corrected), 31 echoed memories mechanically de-headered, all embeddings rebuilt — health now `memory hygiene OK / embedding coverage OK`. Known residual: the same hooks are registered in BOTH `~/.claude/settings.json` and repo `.claude/settings.json`, so njhook sessions double-fire (double injection + duplicate events) — registration topology left for an explicit decision. **RESOLVED by #74** (below).
- **Hook delivery idempotency** (#74, 2026-07-18): closed the double-fire residual by DESIGN rather than registration surgery (removing repo-level entries would break clone-and-it-works; policing topology is fragile). `hooks/dedup.py`: each delivery claims a sha of the raw stdin payload atomically (`O_CREAT|O_EXCL` marker in temp, `HOOKS_DEDUP_WINDOW_SECS=30`); a byte-identical payload within the window is the same dispatch delivered via a second registration and is dropped. Wired into both entry points (`log_event.py`, `inject_memory.py`), all four clients. **Fail-open** (an error can never suppress capture/recall — a lost dedup merely duplicates, today's status quo). E2E-verified: identical dispatch twice → one injection + one `:Event`; distinct dispatch still passes; stale-marker/genuine-resubmit passes after the window.
...
| #27 | merged | Phase E (PR-3 / E1) — LLM judge wired into the nightly (opt-in); new-only contradiction quarantine; acceptance #1 met → **Phase E complete** |
...
- Tests: **19 → 173** over the program (live Neo4j + pure).
