...
| **D — Typed memory + admission gate** | ✅ done (#13,#16,#22,#29,#30,#31) | Structured records; block ungrounded dream output | F3, Gap 3 (13-type vocab), Gap 9 evals | A, C |
...
**Status:** ✅ Done & fully aligned (#13, #16, #22, #29, #30, #31) — all four acceptance bars met. **PR-1**: `:EXTRACTED_FROM` claim-level provenance via heuristic top-K overlap attribution (`dream.attribute_events`; bounded, no explosion). **PR-2**: A-MAC grounding admission gate — `quality.grounding_score` (memory body vs source transcript); a NEW memory below `DREAM_GROUNDING_MIN` (0.10) is routed to `pending_review` (recall hides it; `njhook review` adjudicates — Phase E loop), while updates to existing-active memories are never gated (no clobber). **PR-3 (D3 retrieval eval)**: `dream/eval_retrieval.py` — a deterministic golden-set retrieval eval over the shared `recall.prompt_query` reporting hit@k + MRR; `njhook eval-retrieval`; gated by `tests/test_eval_retrieval.py`. Partially closes acceptance #3 (retrieval half). **PR-4 (D1 typed `kind` vocabulary)**: decision (2026-06-02) — **`kind` IS the semantic type** (replacing the bucket labels, which live only in the PATH prefix; recall buckets by path so ranking is untouched). `hooks/memory_types.py` is the single source: **all 15 types ACTIVE**, no reserved tier; the 4 legacy labels stay valid during a **migration window** (`VALID_KINDS = semantic ∪ legacy`) with `normalize_kind()` mapping legacy→semantic. `kind` is now a first-class enum field in `DREAM_JSON_SCHEMA`, the dream prompt describes the 15 types + the path/kind decoupling, `quality.validate_memory` validates against the single source, and the dream write **stamps `m.kind` as a queryable node property** (the Cypher leg of the round-trip); `njhook stats` reports by bucket AND by kind. Closes acceptance #1 + #4. **PR-5 (D1 migration)**: `njhook migrate-kinds` — bulk re-tag legacy-frontmatter memories to their semantic type (an audited `edit`) + backfill the `m.kind` property where missing; pure rewrite touches only the kind line; idempotent; `--dry-run`. Verified on synthetic legacy data (6 tests) and run on the live graph (which the nightly had already re-dreamed onto semantic kinds → idempotent no-op, 0 legacy remaining). **PR-6 (D3 distillation eval)**: `dream/eval_distillation.py` — hybrid. The deterministic `score()` (structural validity + path-bucket + `kind` enum + grounding + golden-fact coverage) is CI-gated by `tests/test_eval_distillation.py` (9 tests, good-set passes / each degradation fails); the opt-in `njhook eval-distillation --provider …` runs a REAL provider over the golden sessions and applies the same scorer for a per-provider matrix (LLM-in-the-loop → local/manual, not a CI gate). Closes acceptance #3. **Status: Phase D fully aligned.** Open follow-on (not an acceptance bar): model-cited attribution precision. (Caveat: grounding/coverage are coarse token-overlap proxies — strict on very short transcripts; the opt-in run is diagnostic, the deterministic scorer is the regression guard.)

**Goal:** structured records; ungrounded dream output can't enter the graph. (Gap 3, 9.)

**Work items**
- **D1 — Type vocabulary** (`hooks/memory_types.py`, `dream/quality.py`, `dream/prompts.py`, `dream/dream.py`) ✅. `kind` IS the semantic type — **all 15 ACTIVE** (`preference, projectrule, decision, procedure, fact, constraint, toolpattern, incident, openquestion, commitment, goal, context, learning, observation, artifact`), no reserved tier (user decision 2026-06-02, superseding the earlier 9-now plan). The PATH prefix (profile/tools/project/general) is the storage bucket — a separate axis from `kind`. First-class enum in `DREAM_JSON_SCHEMA`; validated at the boundary; stamped as the `m.kind` node property. Legacy bucket-labels accepted during a migration window (`normalize_kind` maps them to semantic types).
...
- **D3 — Eval suites** (`dream/eval_retrieval.py`, `dream/eval_distillation.py`, Gap 9) ✅. **Retrieval** (`njhook eval-retrieval`): deterministic golden query→path set → hit@k + MRR, CI-gated. **Distillation** (`njhook eval-distillation`): deterministic scorer (structure + grounding + golden-fact coverage) CI-gated, plus an opt-in real-provider matrix run. Both wired into the test suite.

**Acceptance bar**
1. `kind` round-trips Python frozenset ↔ JSON-schema enum ↔ Cypher ↔ dashboard; an out-of-vocab kind is rejected by the quality gate. ✅ — `memory_types.MEMORY_KINDS` (frozenset) == `DREAM_JSON_SCHEMA` enum == `quality.VALID_KINDS` source; stamped as `m.kind` (Cypher) + rendered in content/stats; out-of-vocab rejected (`test_memory_types.py`).
...
3. Eval matrix reports pass/fail per provider/model; CI fails on a deterministic semantic regression. ✅ — **retrieval**: `tests/test_eval_retrieval.py` gates hit@5/MRR on a golden set; **distillation**: `tests/test_eval_distillation.py` gates the deterministic scorer (structure + grounding + coverage, good-set passes / each degradation fails), and `njhook eval-distillation --provider …` reports the per-provider matrix (opt-in, LLM-in-the-loop).
4. Legacy memories with only a path-prefix kind still validate (migration window). ✅ — `VALID_KINDS = MEMORY_KINDS ∪ LEGACY_KINDS`; the 4 bucket labels validate until `njhook migrate-kinds` re-tags them (`test_memory_types.py::test_quality_accepts_semantic_and_legacy_rejects_unknown`).
...
**Status:** ✅ Done & fully aligned (#19, #24, #25, #26) — all three acceptance bars met (H1 egress, H2 audit, H4 rehearsal); H3 anti-poisoning shipped as bonus robustness beyond the bars. **PR-4 (H4 — restore rehearsal)**: `njhook rehearse-restore` proves the backup→restore pipeline end-to-end on a disposable marker subgraph (real `cmd_backup` → confirm marker captured → filter to an isolated payload → delete marker → real `cmd_restore` from the backup's own format → verify content + revision lineage → clean up), recording a `:RehearsalRun {ts, ok, detail}`. `health` reports the last rehearsal's age — `warn` if stale (`> NJHOOK_REHEARSAL_DAYS`, default 30) or never run, `fail` if the last one failed. 6 tests (pure health-row branches + a live round-trip that records a run and leaves no marker behind). **This meets acceptance #3.** **PR-3 (H2 — audit log)**: every memory mutation now appends an immutable `:MemoryRevision {ts, operation, actor, status, content_snapshot}` (closed `audit.OPERATIONS` vocab). The dream write already recorded `dream_update`; this PR closes the gap where **review transitions and manual edits overwrote state in place** (recording only the latest `reviewed_at` — losing status-transition history + reviewer attribution). `review.approve/reject/supersede/flag_contradiction` and `njhook edit` now record an entry with the prior status + actor; `audit.trail()` reconstructs a memory's full ordered, attributed history and `audit.recent()` a graph-wide view; surfaced by `njhook audit <path>` / `njhook audit --recent` and on the dashboard history page (no new route — the entries are revisions). Because it reuses the Phase A revision node, the existing `backup`/`restore` round-trips the audit log (test-pinned). **This meets acceptance #2.** 11 tests (incl. the first dashboard test). **PR-1 (H1)**: sensitivity tagging — `privacy.sensitivity_for` classifies events by cwd (`HOOKS_SENSITIVE_PATHS` / `~/.njhook/sensitive.txt`), stamped at capture; egress policy — `dream.egress_blocked` keeps high-sensitivity sessions off remote providers (remote primary skipped; remote fallback suppressed) unless `DREAM_ALLOW_SENSITIVE_EGRESS=1`; `health` reports the policy. **PR-2 (H3)**: anti-poisoning admission gate — `quality.poisoning_risk` quarantines a NEW memory to `pending_review` only when ALL THREE signals fire: directive content (`is_directive`, closed `DIRECTIVE_MARKERS` vocab — stands in for the deferred typed rule/procedure/constraint kinds), thin source session (`< POISON_MIN_EVENTS`), and high novelty (`novelty_score >= POISON_NOVELTY_MIN`, low overlap with existing trusted memory). Wired into `write_memories` alongside the grounding gate; updates to existing-active memories are exempt; skipped when no events were provided (can't assess provenance). 13 tests (closed-vocab + boundary + negative + DB thin/rich/non-directive/exempt). **Remaining**: H2 audit CLI/dashboard (the `:MemoryRevision`/`:DreamRun` log already records mutations — acceptance #2), H4 backup/restore-rehearsal check in health (acceptance #3), and full confidence-*annealing* (corroboration-over-time auto-promotion) deferred as a follow-on.
...
- **H2 — Audit log** ✅ — `:MemoryRevision` is the audit node. **Correction to the original assumption:** it did *not* already record review/edit transitions (those overwrote `m.status`/`m.content` in place); `hooks/audit.py` now records an entry at every mutation site so the chain is complete. Exposed via `njhook audit <path>` / `--recent` and the dashboard history page. Round-trips through `backup`/`restore`.
...
- **H4 — Backup/restore rehearsal check** in `health` ✅ — `njhook rehearse-restore` runs a real backup→restore round-trip on a disposable marker and records a `:RehearsalRun`; `health` reports its age (warn if stale/never, fail if the last failed).
...
2. Every memory mutation is reconstructable from the audit log. ✅ — `audit.record` writes a `:MemoryRevision` at every mutation site (dream write / edit / approve / reject / supersede / flag); `audit.trail` reconstructs the ordered, attributed history; round-trips through backup/restore. *(The mutation — what/when/who — is captured for all sites; the automated-gate **reason**, e.g. grounding vs poisoning quarantine, is in dream logs, not the graph entry.)*
3. `health` reports policy status and restore-rehearsal age. ✅ — egress-policy row (H1) + restore-rehearsal row (H4): age of the last `:RehearsalRun`, warn if stale/never, fail if the last failed. `njhook rehearse-restore` performs the drill via a real backup→restore round-trip.
...
2. **Type vocabulary size:** ~~adopt the full set now, or start with the roadmap's 9 and grow?~~ ~~DECIDED (2026-06-01): ship the 9 `ACTIVE` types now~~ **RE-DECIDED (2026-06-02): all 15 types ACTIVE, no reserved tier; `kind` REPLACES the bucket labels (which move to the PATH axis), with a migration window for legacy rows.** (User chose "replace `kind`" + "all 15 active" — superseding the 9-now plan.)
